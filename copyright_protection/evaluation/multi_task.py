...
class Evaluator(object):
    ...
    def evaluate(self, images, model, weighted_loss, losses, dataset, split):
        ...
        # import itertools
        # if len(weighted_loss) < len(dataset):
        #     cycled_weighted_loss = itertools.cycle(weighted_loss)
        #     weighted_loss = list(itertools.islice(cycled_weighted_loss, len(dataset)))
        ...
        for i in range(num_examples):
            ...
            if isinstance(images, torch.Tensor):
                ...
                name2eval_kwargs[name]['images'].append(
                    (_images[i] if _images is not None else None for _images in images)
                )
            elif images is None:
