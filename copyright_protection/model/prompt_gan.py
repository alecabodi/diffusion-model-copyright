...
class PromptGAN(PromptGANBase):
    ...
    def forward(self, sample_id, prompt=None, alpha=None):

        if prompt is None:
            print('Warning: prompts is None')
            print('setting empty prompts')
            prompt = torch.zeros(sample_id.shape[0], 0, device=self.device)
        ...
        if self.gan_type in ['BigGAN']:
            ...
            img = self.gan_wrapper(z=z, prompt=prompt)
