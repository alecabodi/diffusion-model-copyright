...
class Custom_LPIPS(nn.Module):
    def __init__(self, pretrained=True, net='alex', variant='', version='0.1', lpips=True, spatial=False,
        ...
        super(Custom_LPIPS, self).__init__()
        if(verbose):
        ...
        self.net = get_vae_encoder()
        self.chns = [128, 128, 128, 128, 256, 256, 256, 512, 512, 512, 512, 512]
        self.L = len(self.chns) - 3 # ignore final 3 layers
        ...
        if lpips:
            ...
            self.lins = [self.lin0,self.lin1,self.lin2,self.lin3,self.lin4, self.lin5,self.lin6,self.lin7,self.lin8,self.lin9, self.lin10,self.lin11]
    ...
    def forward(self, in0, in1, retPerLayer=False, normalize=False):
        ...
        for i, output in enumerate(outs0):
            print(f"Slice {i+1}: {output.shape}, dtype: {output.dtype}")
            print(output)
...
def get_vae_encoder():
    ...
    vae = pipe.vae
    vae_encoder_wrapper = VAEEncoderWrapper(vae.encoder)

    return vae_encoder_wrapper

class VAEEncoderWrapper(nn.Module):
    def __init__(self, encoder):
        super(VAEEncoderWrapper, self).__init__()

        # Initialize slices as sequential containers
        self.slices = nn.ModuleList()

        # Add initial conv_in layer as the first slice
        self.slices.append(nn.Sequential(encoder.conv_in))
        ...
        for i, down_block in enumerate(encoder.down_blocks):
            ...
            for resnet in down_block.resnets:
                self.slices.append(resnet)
                print(type(resnet))
            
            # Add Downsampler block\
            if i != len(encoder.down_blocks) - 1:
               self.slices.append(down_block.downsamplers[0])
    ...
    def forward(self, x):
        ...
        # Create a namedtuple to store the outputs with layer names
        Outputs = namedtuple('Outputs', [f'layer{i+1}' for i in range(len(outputs))])
        out = Outputs(*outputs)

        # Convert namedtuple outputs back to float
        out = out._replace(**{field: getattr(out, field).float() for field in out._fields})
