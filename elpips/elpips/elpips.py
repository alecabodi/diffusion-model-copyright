...
def sample_ensemble(config):
	...
	flips = tf.math.floormod(flips, 4)
	flips = tf.random.shuffle(flips)
	flips = flips[:N]
		
	# Sample transposing.
	swap_xy = tf.random.uniform([], minval=0, maxval=2, dtype=tf.int32)

	# Color multiplication.
	def sample_colors():
		color = tf.random.uniform([N], minval=0.0, maxval=1.0, dtype=config.dtype)
		color += tf.cast(tf.range(N), config.dtype)
		color /= tf.cast(N, config.dtype)
		return tf.random.shuffle(color)
	...
	permutations = tf.reshape(tf.random.shuffle(permutations)[:N, :], [-1])
